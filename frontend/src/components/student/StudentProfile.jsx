    ...
    const fetchStudentData = async () => {
        try {
            ...
            if (response.data?.success) {
                ...
                // Prefer the record with the most populated profile fields.
                const profileFields = [
                    'middle_names',
                    'contact_number',
                    'date_of_birth',
                ...
                const completenessScore = (app) =>
                    profileFields.reduce((score, field) => {
                ...
                const bestApp = [...apps].sort((a, b) => {
                    const scoreDiff = completenessScore(b) - completenessScore(a);
                    if (scoreDiff !== 0) return scoreDiff;
                    // Tie-breaker: latest updated record.
                    return new Date(b.updated_at || b.created_at || 0) - new Date(a.updated_at || a.created_at || 0);
                })[0];

                setStudentData(bestApp);
            }
        } catch (error) {
