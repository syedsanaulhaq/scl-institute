...
function assignMoodleRoles($userid, $sclRole) {
    ...
    $normalizedRole = ucwords(strtolower($sclRole));
    ...
    $roleMapping = array(
        ...
        'Student' => 'student',               // Student -> Moodle Student
    );
    ...
    $moodleRole = isset($roleMapping[$normalizedRole]) ? $roleMapping[$normalizedRole] : null;
    
    if (empty($moodleRole)) {
        error_log('[SSO] No role mapping found for SCL role: ' . $sclRole . ' (normalized: ' . $normalizedRole . ')');
        return;
    }
    ...
    // Check if role already assigned
    $existing = $DB->get_record('role_assignments', 
        array('userid' => $userid, 'roleid' => $role->id, 'contextid' => $context->id));
